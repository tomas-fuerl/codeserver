...
require_literal "$ci_workflow" '    runs-on: ubuntu-latest' "CI runner is not explicit"
require_literal "$ci_workflow" '    timeout-minutes: 45' "CI timeout missing"
require_literal "$ci_workflow" '  cancel-in-progress: true' "CI concurrency cancellation missing"
require_literal "$ci_workflow" '          platforms: linux/amd64' "CI build platform missing"
require_literal "$ci_workflow" '          push: false' "CI build must not push"
require_literal "$ci_workflow" '          load: false' "CI build must not load"
